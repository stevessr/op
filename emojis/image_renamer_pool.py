...
class ImageRenamer:
    def __init__(self, api_key: str, model_name: str = "gemini-2.0-flash-exp", base_url: Optional[str] = None,
                 ...
                 max_workers: int = 24): # 新增 max_workers 参数
        """
        初始化图片重命名器
        ...
        self.max_workers = max_workers # 存储 max_workers
        self.setup_gemini()
        ...
        self.processing_lock = threading.Lock() # 用于保护共享数据（重命名历史和已处理文件集合）的线程锁
    ...
    def setup_gemini(self):
        ...
        try:
            ...
            if self.base_url:
                # 推荐使用 GOOGLE_API_BASE 环境变量来配置自定义端点
                os.environ['GOOGLE_API_BASE'] = self.base_url
                logger.info(f"设置自定义API端点: {self.base_url} (通过环境变量 GOOGLE_API_BASE)")
    ...
    def get_all_images(self, root_dir: str) -> List[Path]:
        ...
        images = []
        skipped_files_during_scan = [] # 扫描阶段跳过的文件列表
        ...
        for file_path in root_path.rglob('*'):
            if file_path.is_file() and self.is_image_file(file_path):
                ...
                if should_skip:
                    skipped_files_during_scan.append((file_path, skip_reason))
                    continue

                images.append(file_path)

        logger.info(f"找到 {len(images)} 个图片文件")
        if skipped_files_during_scan:
            logger.info(f"扫描阶段跳过 {len(skipped_files_during_scan)} 个文件:")
            for file_path, reason in skipped_files_during_scan[:10]:  # 只显示前10个
                logger.info(f"  - {file_path.name}: {reason}")
            if len(skipped_files_during_scan) > 10:
                logger.info(f"  ... 还有 {len(skipped_files_during_scan) - 10} 个文件在扫描阶段被跳过")
    ...
    def analyze_image(self, image_path: Path) -> Optional[str]:
        ...
        """
        try:
            # 如果提供了自定义base_url，且它不是官方API端点，则使用requests直接调用
            # 这种方式对于代理或自定义部署更可靠
            if self.base_url and "api.gen.ai" not in self.base_url: 
    ...
    def _analyze_image_with_custom_url(self, image_path: Path) -> Optional[str]:
        ...
        # 构建URL，假设自定义URL的格式是 base_url/models/model_name:generateContent
        url = f"{self.base_url.rstrip('/')}/models/{self.model_name}:generateContent"
        ...
        logger.warning(f"自定义API返回错误: {response.status_code} - {response.text} for {image_path.name}")
        return None
    ...
    def rename_image(self, image_path: Path, new_name: str, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            if dry_run:
                ...
                # 在试运行模式下，如果会发生重命名，也将其添加到已处理文件集合，以避免后续的真实处理
                ...
                with self.processing_lock:
                    ...
                    # 更新已处理文件集合
                    self.processed_files.add(str(image_path.resolve()))
                    self.processed_files.add(str(new_path.resolve())) # 也把新文件名加进去，防止重复处理新文件
                return True
    ...
    def _process_single_image(self, image_path: Path, force_reprocess: bool, dry_run: bool) -> Tuple[int, str]:
        """
        用于线程池的辅助方法，处理单张图片。
        返回 (状态码, 消息)
        状态码: 0=成功, 1=错误, 2=跳过 (已处理/无需更改)
        """
        try:
            # 检查文件是否已经被处理过（除非强制重新处理）
            if not force_reprocess and self.is_file_processed(image_path):
                logger.info(f"跳过文件（已处理过）: {image_path.name}")
                return 2, f"跳过文件（已处理过）: {image_path.name}"
            ...
            if description:
                ...
                if new_name == image_path.name:
                    ...
                    # 即使没有重命名，如果文件被分析过且无需更改，也将其视为已处理，避免后续重复分析
                    if not dry_run: # 仅在非试运行模式下更新 processed_files
                        with self.processing_lock:
                            self.processed_files.add(str(image_path.resolve()))
                    return 2, f"跳过文件（文件名无需更改）: {image_path.name}"

                # 重命名文件
                if self.rename_image(image_path, new_name, dry_run):
                    return 0, f"处理成功: {image_path.name} -> {new_name}"
                else:
                    return 1, f"重命名失败: {image_path.name}"
            else:
                logger.warning(f"跳过文件（无法分析）: {image_path.name}")
                return 1, f"无法分析: {image_path.name}"
                
        except Exception as e:
            logger.error(f"处理文件时出错 {image_path}: {e}")
            return 1, f"处理异常: {image_path.name} - {e}"
    ...
    def process_directory(self, directory: str, dry_run: bool = False,
        ...
        logger.info(f"使用线程池，最大工作线程数: {self.max_workers}")
        ...
        error_count = 0
        skipped_count = 0
        total_files_to_process = len(images)

        # 使用 ThreadPoolExecutor 进行并发处理
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # 提交所有图片处理任务
            futures = {executor.submit(self._process_single_image, image_path, force_reprocess, dry_run): image_path for image_path in images}
            
            # 迭代已完成的 future
            for i, future in enumerate(concurrent.futures.as_completed(futures), 1):
                image_path = futures[future] # 从 future 映射中获取原始路径
                logger.info(f"处理进度: {i}/{total_files_to_process} - {image_path.name}")
                try:
                    status_code, message = future.result() # 获取任务结果
                    if status_code == 0:
                        success_count += 1
                    elif status_code == 1:
                        error_count += 1
                        ...
                        skipped_count += 1
                    # 辅助函数已记录详细消息，此处只需更新计数
                except KeyboardInterrupt:
                    logger.info("用户中断处理")
                    # 在中断时，executor 会尝试取消未完成的任务并等待已开始的任务完成
                    break 
                except Exception as e:
                    logger.error(f"获取文件 {image_path} 的结果时出错: {e}")
                    error_count += 1
        
        logger.info(f"处理完成! 成功: {success_count}, 失败: {error_count}, 跳过: {skipped_count}")

        # 保存重命名历史 (仅在非试运行模式下且发生过重命名时)
    ...
    def load_rename_history(self):
        ...
        try:
            ...
            if history_file.exists():
                ...
                with self.processing_lock: # 加锁保护共享数据的初始化
                    self.rename_history = history_data
                    # 构建已处理文件集合
            ...
            with self.processing_lock: # 即使加载失败，也确保数据被清空且锁保护
                self.rename_history = []
    ...
    def is_file_processed(self, file_path: Path) -> bool:
        ...
        try:
            ...
            with self.processing_lock: # 加锁保护读取操作
    ...
    def save_rename_history(self):
        ...
        try:
            history_file = Path('rename_history.json')
            with self.processing_lock: # 在保存前加锁，确保数据一致性
                data_to_save = list(self.rename_history) # 复制一份，避免在保存过程中被其他线程修改
...
def main():
    ...
    parser.add_argument('--workers', type=int, default=os.cpu_count() * 2, 
                        help='线程池最大工作线程数 (默认: CPU核心数 * 2)') # 新增 workers 参数
    parser.add_argument('--delay', type=float, default=1.0, help='API调用间隔（秒）')
    # 移除了 --delay 参数，因为在线程池环境下其作用不明显且可能导致误解
    ...
    try:
        ...
        renamer = ImageRenamer(
            ...
            max_workers=args.workers # 传递 max_workers 参数
        )
        ...
        logger.error(f"程序执行失败: {e}", exc_info=True) # 打印完整 traceback
        sys.exit(1)
